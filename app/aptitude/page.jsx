...
export default function AptitudePage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-purple-50 to-pink-50 dark:from-black dark:via-zinc-950 dark:to-black p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        {user && (
          <div className="bg-white dark:bg-zinc-900 rounded-xl shadow-sm p-4 mb-6 border border-gray-100 dark:border-zinc-800">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-500 dark:text-zinc-400 truncate max-w-[200px] sm:max-w-none">{user.email}</div>
                </div>
              </div>
              <button
                onClick={() => setShowHistory(!showHistory)}
                className="w-full sm:w-auto px-4 py-2 bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400 rounded-lg hover:bg-purple-200 dark:hover:bg-purple-900/50 transition-colors flex items-center justify-center gap-2"
              >
        ...
        {showHistory && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-2 sm:p-4">
            <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl w-full max-w-3xl max-h-[95vh] sm:max-h-[90vh] overflow-hidden flex flex-col">
              <div className="p-4 sm:p-6 border-b border-gray-200 dark:border-zinc-800 flex items-start justify-between">
                <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                  <div className="p-2 bg-purple-100 dark:bg-purple-900/30 rounded-lg flex-shrink-0">
                    <Trophy className="w-5 h-5 sm:w-6 sm:h-6 text-purple-600 dark:text-purple-400" />
                  </div>
                  <div className="min-w-0">
                    <h2 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white truncate">Your Test History</h2>
                    <p className="text-xs sm:text-sm text-gray-600 dark:text-zinc-400">Track your progress and performance</p>
                  </div>
                </div>
                <button
                  onClick={() => setShowHistory(false)}
                  className="p-2 hover:bg-gray-100 dark:hover:bg-zinc-800 rounded-lg transition-colors flex-shrink-0 ml-2"
                >
              ...
              <div className="flex-1 overflow-y-auto p-3 sm:p-6">
                {historyLoading ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-600 dark:text-zinc-400">Loading your history...</p>
                  </div>
                  ...
                  <div className="space-y-3 sm:space-y-4">
                    {userHistory.map((attempt, index) => {
                      ...
                      return (
                        <div
                          key={attempt.id}
                          className={`p-3 sm:p-5 rounded-xl border-2 transition-all hover:shadow-md ${getScoreBgColor(attempt.score, attempt.totalMarks)} dark:bg-zinc-800 dark:border-zinc-700`}
                        >
                          <div className="flex items-start justify-between mb-3 gap-2">
                            <div className="flex-1 min-w-0">
                              ...
                              <h3 className="font-semibold text-base sm:text-lg text-gray-900 dark:text-white line-clamp-2">{attempt.practiceSetTitle}</h3>
                              <p className="text-xs sm:text-sm text-gray-600 dark:text-zinc-400 mt-1">{formatDate(attempt.completedAt)}</p>
                            </div>
                            <div className="text-right flex-shrink-0">
                              <div className={`text-2xl sm:text-3xl font-bold ${getScoreColor(attempt.score, attempt.totalMarks)}`}>
                                {attempt.score}
                                <span className="text-base sm:text-lg text-gray-400 dark:text-zinc-500">/{attempt.totalMarks}</span>
                              </div>
                              <div className="text-xs sm:text-sm font-medium text-gray-600 dark:text-zinc-400 mt-1">
                                {percentage}%
                              </div>
                            </div>
                          </div>
                  ...
                  <div className="text-center py-12">
                    <div className="w-16 h-16 sm:w-20 sm:h-20 bg-gray-100 dark:bg-zinc-800 rounded-full flex items-center justify-center mx-auto mb-4">
                      <Trophy className="w-8 h-8 sm:w-10 sm:h-10 text-gray-300 dark:text-zinc-600" />
                    </div>
                    <h3 className="text-base sm:text-lg font-semibold text-gray-900 dark:text-white mb-2">No attempts yet</h3>
                    <p className="text-sm sm:text-base text-gray-600 dark:text-zinc-400 mb-6">Start practicing to track your progress!</p>
                    <button
                      onClick={() => setShowHistory(false)}
              ...
              {userHistory.length > 0 && (
                <div className="p-4 sm:p-6 border-t border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-800">
                  <div className="grid grid-cols-3 gap-2 sm:gap-4">
                    <div className="text-center">
                      <div className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">{userHistory.length}</div>
                      <div className="text-xs sm:text-sm text-gray-600 dark:text-zinc-400">Total Attempts</div>
                    </div>
                    <div className="text-center">
                      <div className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">{Object.keys(userAttempts).length}</div>
                      <div className="text-xs sm:text-sm text-gray-600 dark:text-zinc-400">Sets Completed</div>
                    </div>
                    <div className="text-center">
                      ...
                      <div className="text-xs sm:text-sm text-gray-600 dark:text-zinc-400">Avg Score</div>
                    </div>
                  </div>
                </div>
              )}
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Aptitude Practice</h1>
          </div>
          <p className="text-gray-600 dark:text-zinc-400">Master aptitude with comprehensive practice sets</p>
        </div>
              ...
              return (
                <button
                  ...
                  className={`p-4 rounded-xl transition-all duration-300 ${selectedCategory === category.id
                    ...
                    : 'bg-white dark:bg-zinc-900 text-gray-700 dark:text-zinc-300 hover:shadow-md border border-gray-200 dark:border-zinc-800'
                    }`}
                >
                  <Icon className={`w-6 h-6 mx-auto mb-2 ${selectedCategory === category.id ? 'text-white' : 'text-gray-600 dark:text-zinc-400'}`} />
                  <div className="text-sm font-medium text-center">
                    {category.name}
                  </div>
                  {category.id !== 'all' && questionCounts[category.id] !== undefined && (
                    <div className={`text-xs mt-1 ${selectedCategory === category.id ? 'text-white/80' : 'text-gray-500 dark:text-zinc-500'}`}>
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-xl p-4 border border-gray-100 dark:border-zinc-800 shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-sm text-gray-600 dark:text-zinc-400">Practice Sets</div>
              </div>
            </div>
          </div>
          <div className="bg-white dark:bg-zinc-900 rounded-xl p-4 border border-gray-100 dark:border-zinc-800 shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-sm text-gray-600 dark:text-zinc-400">You Completed</div>
              </div>
            </div>
          </div>
          <div className="bg-white dark:bg-zinc-900 rounded-xl p-4 border border-gray-100 dark:border-zinc-800 shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-sm text-gray-600 dark:text-zinc-400">Total Attempts</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {filteredSets.map((set) => {
            ...
            return (
              <div
                key={set.id}
                className={`bg-white dark:bg-zinc-900 rounded-xl border border-gray-100 dark:border-zinc-800 shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden group hover:-translate-y-1 ${!available ? 'opacity-75' : ''}`}
              >
                <div className="p-5">
                  ...
                  <h3 className="font-semibold text-lg line-clamp-2 text-gray-900 dark:text-white mb-4">
                    {set.title}
                  </h3>

                  <div className="flex items-center gap-4 mb-4 text-sm text-gray-600 dark:text-zinc-400">
                  ...
                  {attempted && userScore ? (
                    <div className="mb-4 p-3 bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 rounded-lg border border-blue-100 dark:border-blue-900">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-sm text-gray-600 dark:text-zinc-400 font-medium">Your Best Score</span>
                    ...
                    <div className="mb-4 p-3 bg-gray-50 dark:bg-zinc-800 rounded-lg border border-gray-200 dark:border-zinc-700">
                      <div className="flex items-center justify-between">
                        <span className="text-sm text-gray-600 dark:text-zinc-400 font-medium">Total Marks</span>
        ...
        {filteredSets.length === 0 && (
          <div className="text-center py-16">
            <Target className="w-12 h-12 text-gray-300 dark:text-zinc-600 mx-auto mb-4" />
            <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">No practice sets found</h3>
            <p className="text-gray-500 dark:text-zinc-400">Try selecting a different category</p>
          </div>
        )}
      </div>
    </div>

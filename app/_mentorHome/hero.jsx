...
const HeroSection = () => {
    ...
    useEffect(() => {
        if (!isLoading && user) {
            const email =
                user.email ||
                user.given_name || // fallback if Kinde doesn't return email
                user.id;

            if (!email) {
                console.error("❌ No email found in Kinde user object");
                return;
            }
    ...
    // --------------------------------------------------------
    // 2) Auto-open modal ONLY after login AND email available
    // --------------------------------------------------------
    useEffect(() => {
        if (isLoading) return;

        // If not logged in, clear flag and exit
        if (!user) {
            sessionStorage.removeItem("openMeetSeniorModal");
            return;
        }
        ...
        const shouldOpen = sessionStorage.getItem("openMeetSeniorModal");
        if (shouldOpen === "true") {
            sessionStorage.removeItem("openMeetSeniorModal");
            setShowRegistrationModal(true);
        }
    }, [isLoading, user, userEmail]);


    // ------------------------------------------
    // 3) Meet Senior button handler
    // ------------------------------------------
    const handleMeetSenior = () => {
        ...
        // Logged in but email not extracted yet
        const email = emailRef.current || userEmail;
        if (!email) {
            alert("Unable to load your account details. Please re-login.");
            return;
        }
    ...
    // ------------------------------------------
    // 4) Close + Success handlers
    // ------------------------------------------
    const handleCloseModal = () => setShowRegistrationModal(false);

    // Add this to your handleRegistrationSuccess function
    ...
    return (
        <>
            {/* ✅ ONLY render modal if ALL conditions are met */}
            {user && !isLoading && showRegistrationModal && (
                <RegistrationModal
                    userEmail={emailRef.current || userEmail}
                    ...
                    <div className="grid md:grid-cols-2 gap-12 items-center">
                        ...
                        <div className="space-y-6 animate-fade-in">
                            <div className="mb-4 inline-block">
                                <div className="relative px-6 py-3 bg-white/5 dark:bg-white/5 backdrop-blur-sm rounded-2xl border border-white/20 dark:border-white/10 shadow-2xl">
                                    <h2 className="text-4xl md:text-5xl font-black tracking-wider" style={{ fontFamily: "'Fredoka One', 'Righteous', 'Rubik Mono One', cursive", letterSpacing: "0.05em" }}>
                                    ...
                                    <div className="absolute top-0 left-0 w-full h-full rounded-2xl bg-gradient-to-r from-[#2663EB]/20 to-red-500/20 blur-xl -z-10"></div>
                                </div>
                            </div>
                            <h1
                            ...
                            <div className="flex flex-col sm:flex-row gap-4 pt-4">
                                <button
                                    ...
                                    disabled={isLoading}
                                    className="group relative overflow-hidden cursor-pointer bg-gradient-to-r from-red-500 to-pink-600 dark:from-red-600 dark:to-pink-700 text-white px-8 py-4 rounded-2xl font-bold text-lg transition-all duration-300 hover:shadow-2xl hover:shadow-red-500/50 dark:hover:shadow-red-500/30 hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
                                    style={{ fontFamily: "'Space Grotesk', 'Outfit', sans-serif" }}
                                >
                                    <span className="relative z-10 flex items-center justify-center gap-2">
                                        {isLoading ? "Loading..." : "Meet Your Senior"}
                                        {!isLoading && (
                                            <svg
                            ...
                            <div className="flex items-center gap-6 pt-6">
                                <div className="flex -space-x-3">
                                    <img src="https://api.dicebear.com/7.x/avataaars/svg?seed=student1" alt="Student" className="w-10 h-10 rounded-full border-2 border-white dark:border-black" />
                                    <img src="https://api.dicebear.com/7.x/avataaars/svg?seed=student2" alt="Student" className="w-10 h-10 rounded-full border-2 border-white dark:border-black" />
                                    <img src="https://api.dicebear.com/7.x/avataaars/svg?seed=student3" alt="Student" className="w-10 h-10 rounded-full border-2 border-white dark:border-black" />
                                    <img src="https://api.dicebear.com/7.x/avataaars/svg?seed=student4" alt="Student" className="w-10 h-10 rounded-full border-2 border-white dark:border-black" />
                                </div>
                            ...
                            <div className="relative w-full h-[500px]">
                                ...
                                <div className="absolute top-10 right-0 w-80 h-96 card-theme shadow-2xl dark:shadow-blue-500/10 p-6 transform rotate-2 hover:rotate-0 transition-all duration-500 hover:scale-105">
                                    <div className="flex items-center gap-3 mb-4">
                                        <img src="https://api.dicebear.com/7.x/avataaars/svg?seed=senior-mentor" alt="Senior Mentor" className="w-12 h-12 rounded-full animate-pulse" />
                                        <div>
                                ...
                                <div className="absolute top-0 left-0 w-64 card-theme shadow-xl p-4 transform -rotate-3 hover:rotate-0 transition-all duration-500 hover:scale-105 animate-float">
                                    <div className="flex items-center gap-2 mb-2">
                                        <img src="https://api.dicebear.com/7.x/avataaars/svg?seed=junior-student" alt="Junior Student" className="w-8 h-8 rounded-full" />

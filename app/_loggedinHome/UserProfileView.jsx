...
// Mock badge definitions (you'll import from your actual badge system)
const BADGE_DEFINITIONS = {
    yearBadges: {
        fresher: { id: 'fresher_1st_year', name: 'Fresher', subtitle: '1st Year', icon: User, color: 'bg-gradient-to-r from-green-500 to-emerald-600' },
        sophomore: { id: 'sophomore_2nd_year', name: 'Sophomore', subtitle: '2nd Year', icon: User, color: 'bg-gradient-to-r from-blue-500 to-cyan-600' },
        senior: { id: 'senior_3rd_year', name: 'Senior', subtitle: '3rd Year', icon: User, color: 'bg-gradient-to-r from-purple-500 to-pink-600' },
        graduate: { id: 'graduate_4th_year', name: 'Graduate', subtitle: '4th Year', icon: User, color: 'bg-gradient-to-r from-orange-500 to-red-600' }
    },
    achievementBadges: {
        verifiedUser: { id: 'verified_user', name: 'Verified User', subtitle: 'Authenticated', icon: CheckCircle2, color: 'bg-gradient-to-r from-blue-500 to-blue-600' },
        mentor: { id: 'mentor', name: 'Mentor', subtitle: 'Helpful Guide', icon: Star, color: 'bg-gradient-to-r from-rose-500 to-red-600' },
        superMentor: { id: 'super_mentor', name: 'Super Mentor', subtitle: 'Expert Helper', icon: Star, color: 'bg-gradient-to-r from-purple-500 to-pink-600' },
        eliteMentor: { id: 'elite_mentor', name: 'Elite Mentor', subtitle: 'Top Contributor', icon: Star, color: 'bg-gradient-to-r from-yellow-500 to-orange-600' }
    },
    activityBadges: {
        activeParticipant: { id: 'active_participant', name: 'Active Participant', subtitle: 'Community Member', icon: Award, color: 'bg-gradient-to-r from-green-500 to-teal-600' }
    }
};

const assignBadges = (userActivity) => {
    const badges = [];

    // Year badge
    if (userActivity.year === 1) badges.push(BADGE_DEFINITIONS.yearBadges.fresher);
    else if (userActivity.year === 2) badges.push(BADGE_DEFINITIONS.yearBadges.sophomore);
    else if (userActivity.year === 3) badges.push(BADGE_DEFINITIONS.yearBadges.senior);
    else if (userActivity.year === 4) badges.push(BADGE_DEFINITIONS.yearBadges.graduate);

    // Verified badge
    if (userActivity.verified) badges.push(BADGE_DEFINITIONS.achievementBadges.verifiedUser);
...
export default function UserProfileView({ userData }) {
    const handleAskDoubt = () => {
        alert("This would open a messaging interface or doubt submission form!");
    };
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
            {/* Full Width Banner Image Section - Like YouTube */}
            ...
            {/* Main Content Container with Negative Margin to Overlap Banner */}
                                    ...
                                    <div className="flex justify-center -mt-12 mb-4">
                                        <div className="relative">
                                            <div className="w-24 h-24 rounded-full ring-4 ring-white border-2 border-blue-200 overflow-hidden bg-gradient-to-br from-blue-500 to-cyan-500">
                                                <img
                                                    src={userData.profileImageUrl || userData.avatar || "https://api.dicebear.com/7.x/avataaars/svg?seed=default"}
                                                    alt={userData.name}
                                                    className="w-full h-full object-cover"
                                                />
                                            </div>
